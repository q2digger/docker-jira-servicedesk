...
RUN set -x \
    ...
    && curl -Ls                "https://www.atlassian.com/software/jira/downloads/binary/atlassian-servicedesk-${JIRA_VERSION}.tar.gz" | tar -xz --directory "${JIRA_INSTALL_DIR}" --strip-components=1 --no-same-owner \
    ...
    && curl -Ls                "https://jdbc.postgresql.org/download/postgresql-42.2.5.jar" -o "${JIRA_INSTALL_DIR}/lib/postgresql-42.2.5.jar" \
